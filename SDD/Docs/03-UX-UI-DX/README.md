...
Índice navegable de la categoría 03 para Sai-Service-Core: el panel de control web de un servicio que monitorea un SAI, ejecuta el apagado ordenado con reencendido garantizado, administra el parque y expone una API REST. Un solo administrador, acceso por LAN. Layout aplanado (caso de un solo proyecto): los artefactos viven en `SDD/Docs/03-UX-UI-DX/`, sin subnivel `Proyectos/`.
...
| `Wireframes-Alta-Del-Parque-v1.0.md` | Catálogo e inventario, descubrimiento del dispositivo, siembra de verificaciones. Origen CU-02 / UF-1. | Borrador |
...
| Acceso-Login | CU-01 | vacío (N/A), cargando, con datos, error (credenciales / restricción temporal / formulario vencido), identidad creada, secreto actualizado, sesión expirada |
...
| Alta-Del-Parque | CU-02 | vacío (sin dispositivos), cargando, con datos, error (conexión / dato inválido), descubierto sin marca |
